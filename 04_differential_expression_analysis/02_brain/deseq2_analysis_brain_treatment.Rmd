...
This revealed **9103** genes expressed in all samples (approximately 79% of all genes (n = 11545) expressed in the brain).   
Second, density plots were generated for the percentage of expressed genes per sample for:
- Caste (Fig. 2A)
- Treatment (Fig. 2B)
- Stage (Fig. 2C)  
There was a significant difference  in terms of the mean percentage of expressed genes across castes (t-test; p = 0.01), but no difference between treatments (ANOVA; p = 0.38) and stage (ANOVA; p = 0.20).
...
write.table(x = dds_htseq_counts,
            file = "results/all_brain_counts.txt",
            row.names = TRUE,
            col.names = TRUE,
            quote = FALSE,
            sep = "\t")
...
pdf(file = "results/brain_filtered_pheatmap.pdf",
    height = 10,
    width = 10)
brain_heatmap_filtered <- pheatmap(sample_dist_matrix,
                                   clustering_distance_rows = sample_dists,
                                   clustering_distance_cols = sample_dists,
                                   col = colors)
dev.off()
...
_**Fig. 5.** Principal component analysis with first two principal components shown. Individuals are coloured by treatment._  
...
combined_stage_plot <- ggarrange(stage_plot_pc1_2,
                                 ...
                                 common.legend = TRUE)
```
_**Fig. 6.** Principal component analysis with first two principal components shown. Individuals are coloured by stage ovarian development_    
...
ggsave(filename = "results/brain_filtered_pca_plot.png",
       height = 8,
       width = 8)
```
...
```{r, message = FALSE}
lintr::lint(file = "deseq2_analysis_brain_treatment.Rmd")
```