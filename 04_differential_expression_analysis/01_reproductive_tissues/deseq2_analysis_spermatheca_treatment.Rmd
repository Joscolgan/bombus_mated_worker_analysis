---
author: "Joe Colgan"
title: "**Transcriptomic analysis of control and inseminated spermathecae, vagina, and median oviduct of _Bombus terrestris_ workers and queens**"
output:
...
## Introduction
The purpose of the present analysis is to examine similarities and differences in terms of gene expression associated with the spermatheca, vagina, and median oviduct of _Bombus terrestris_ workers and queens. For the present study, control (unmated) and artificially inseminated bumblebees were collected and spermatheca removed at three different time points (two, four and eight days post-treatment).
...
write.table(x = dds_htseq_counts,
            file = "results/all_spermatheca_counts.txt",
            row.names = TRUE,
            col.names = TRUE,
            quote = FALSE,
            sep = "\t")
...
## Rename rownames:
vsd$treatment_group <- gsub(pattern = "Insemination",
...
pdf(file = "results/spermetheca_filtered_pheatmap.pdf",
    height = 10,
    width = 10)
spermatheca_heatmap_filtered <- pheatmap(sampleDistMatrix,
...
nrow(subset(x = worker_results_df,
            padj < 0.05 &
            absLog2FC >= 1))
genes_of_interest <- row.names(subset(x = worker_results_df,
            padj < 0.05 &
            absLog2FC >= 1))
nrow(subset(x = worker_results_df,
            padj < 0.05 &
...
## Normalise counts:
normalised_counts <- t(scale(t(counts(dds_object)),
                             center = TRUE,
                             scale = TRUE))
...
## Update levels for plotting:
input$sample <- as.factor(input$sample)
...
write.table(x = queen_results_df,
...
comp_counts <- as.data.frame(table(c(rownames(worker_object_results_sig),
                         rownames(queen_object_results_sig))))
...
combined_df <- as.data.frame(cbind(row.names(worker_object_results_reordered),
...
combined_df$worker_rank <- worker_object_results_reordered$rank
combined_df$queen_rank <- row.names(combined_df)

combined_df$V2 <- as.numeric(as.character(unlist(combined_df$V2)))
combined_df$V3 <- as.numeric(as.character(unlist(combined_df$V3)))
combined_df$V4 <- as.numeric(as.character(unlist(combined_df$V4)))
combined_df$V5 <- as.numeric(as.character(unlist(combined_df$V5)))
```
...
```{r, message = FALSE}
## Lastly, generate a heatmap for normalised gene expression for genes
## consistently differentially expressed in both castes:
## Use all individuals:
dds_htseq <- DESeqDataSetFromHTSeqCount(sampleTable = samples_info,
                                       directory = directory,
                                       design = ~ caste +
...
## Subset rows for genes of interest:
normalised_counts_unique_degs <- subset(normalised_counts_df,
                                        row.names(normalised_counts_df) %in% comp_counts_conserved$Var1)
...
input$sample <- gsub(pattern = ".ReadsPerGene.out.tab",
                                replacement = "",
                                input$sample)
## Update levels for plotting:
input$sample <- factor(input$sample,
...
samples_info_sorted <- samples_info[with(samples_info,
                                                order(samples_info$stage,
                                                      samples_info$treatment_group)), ]

input$sample <- factor(input$sample,
                          levels = unique(samples_info_sorted$short_name))
dim(input)
...
for (name in 1:nrow(input)){
  #print(input$sample[[name]])
  tmp <- as.character(unlist(input$sample[[name]]))
  input$test[[name]]    <-            c(paste(subset(x = samples_info,
                          short_name == tmp)$treatment_group,
                   " ",
                  subset(x = samples_info,
                          short_name == tmp)$caste,
                   " (",
                   subset(x = samples_info,
                          short_name == tmp)$stage,
                   " (",
...
ggsave(file = "results/conserved_degs_across_castes.pdf",
       height = 6,
       width= 10,
       dpi = 600)
