...
data <- readRDS(file = "input/vsd_ovary_treatment.rds")
...
samples_information <- read.table(file = "data/sample_info_ovary.txt",
                                  header = FALSE)
## Remove tissue:
...
save(exp_MEs, exp_moduleLabels, exp_moduleColors, exp_geneTree, file = "results/ovary-02-networkConstruction-stepByStep.RData")
```
...
# Save module colors and labels for use in subsequent parts
save(exp_MEs, exp_moduleTraitCor, samples_information, exp_textMatrix, file = "results/ovary-03-input_for_labelled_heatmap.RData")
```
...
save(exp_geneModuleMembership,
     ...
     file = "results/spermatheca-04-input_for_membership_scatterplot_red.RData")
```
...
blue_module_genes <- names(transposed_data_filt)[exp_moduleColors=="blue"]
length(blue_module_genes)
write(x = blue_module_genes,
      file = "results/blue_module_genes.txt")
