---
author: "Joe Colgan"
title: "**Weight gene co-expression network analysis of brains of control and inseminated _Bombus terrestris_ workers and queens**"
output:
...
data <- readRDS(file = "input/vsd_brain_treatment.rds")
...
samples_information <- read.table(file = "data/sample_info_brain.txt",
                                  header = FALSE)
## Remove tissue:
...
samples_information <- subset(samples_information,
                              !(rownames(samples_information) %in%
                                  c("BtwC1a_B2.ReadsPerGene.out.tab",
                                    "BtwC1a_B3.ReadsPerGene.out.tab",
                                    "BtQc_B4.ReadsPerGene.out.tab")))
```
...
saveRDS(object = exp_sample_tree2,
        file = "results/exp_sample_tree2_brain.rds")
saveRDS(object = exp_trait_colors,
        file = "results/exp_trait_colors_brain.rds")

save(transposed_data_filt,
     samples_information,
     file = "results/brain-01-dataInput.RData")
```
...
We now calculate the adjencies, using the soft thresholding power 7:
...
To see what the merging did to our module colours, we plot the gene dendrogram again, with the original and merged module colours underneath.
...
exp_gene_module_membership <- as.data.frame(cor(transposed_data_filt,
                                         exp_mes,
                                         use = "p"))
exp_mmp_value <- as.data.frame(corPvalueStudent(as.matrix(exp_gene_module_membership),
                                          exp_nSamples))

names(exp_gene_module_membership) <- paste("MM",
                                          exp_mod_names,
                                          sep = "")
names(exp_mmp_value) <- paste("p.MM",
                            exp_mod_names,
                            sep = "")
...
exp_gsp_value <- as.data.frame(corPvalueStudent(as.matrix(exp_gene_trait_significance),
                                          exp_n_samples))
names(exp_gene_trait_significance) <- paste("GS.",
                                         names(treatment),
                                         sep = "")
names(exp_gsp_value) <- paste("p.GS.",
                             names(treatment),
                             sep = "")
```
...
```{r, echo = FALSE, message = FALSE, results = 'hide', warning=FALSE}
exp_module <- "purple"
exp_column <- match(exp_module, exp_mod_names)
...
save(exp_gene_module_membership,
     ...
     file = "results/brain-04-input_for_membership_scatterplot_red.RData")
```

8. Run lint:

```{r, message = FALSE}
lintr::lint(file = "run_wgcna_brain.Rmd")
```
