...
## Samples:  
The 'fat body' dataset consists of 74 samples:    
**Queen samples:** n = 26    
...
This revealed **8603** genes expressed in all samples (approximately 82% of all genes (n = 11444) expressed in the fat bodies).   
Second, density plots were generated for the percentage of expressed genes per sample for:
- Caste (Fig. 2A)
- Treatment (Fig. 2B)
- Stage (Fig. 2C)  
There was no significant difference  in terms of the mean percentage of expressed genes across castes (t-test; p = 0.16), and treatments (ANOVA; p = 0.2) but there was between individuals collected at different stages (ANOVA; p < 0.05).  
...
To examine further similarities in terms of expression profiles across samples, a principal component analysis using VST gene values was performed. The first principal component, which explains 30% of the variance in the dataset separates two individuals from the rest. Both individuals come from the "injection" group, which is seen in other tissue types.  
```{r, echo = FALSE, message = FALSE, results = 'hide', warning=FALSE}
...
Based on the cluster analyses using full samples (distance matrix Fig.3; PCA Fig. 4), there are two samples that PC1 (30% of variance) separates from the others. Post-removal of the aberrant samples, PC1 explains 41% of the variance separating stage IV individuals from the rest. PC2 (26%) separates samples from stage I and II workers from the rest suggestive of early caste differences in term of ovarian development which reduce between the castes over developmental time. There is no clear pattern associated with treatment.      
...
From this model, a total of **3104** genes were identified as significantly differentiated expressed (adjusted _p_ value < 0.05; Fig. 6) across treatments.  
...
_p_ value and a minimum of absolute log2 fold change of 1, our model identifies **301** (Fig. 7a)
significantly differentially expressed genes with the minority (n = **109**) having elevated 
...
**Comparison of dilutant group and control bees**: Our model identifies **87** (Fig. 7b)
significantly differentially expressed genes with the majority (n = **48**) having elevated 
expression in the fat bodies of bees inseminated only with dilutant.  

**Comparison of the "nothing" group and control bees**: Our model identifies **185** (Fig. 7c)
significantly differentially expressed genes with the majority (n = **153**) having elevated 
...
Analysis identified **3104** genes as differentially expressed (adjusted _p_ value < 0.05) between stage depending on caste. This however could be a consequence of the increased number of worker samples, including more divergent groupings.  
...
Using all queen samples, **940** genes were differentially expressed between inseminated and control bees (Fig. 9B), which was redued to **84** when applying a more stringent filter (Log2FC >= 1).  
...
## Euler plot:  
To visualise similarities in terms of differential expression across bumblebee castes in response to insemination, we used a euler plot. First, in response to insemination, workers and queens share **6** differentially expressed genes, which consists of approximately 3% and 7.7% of worker and queen differentially expressed genes, respectively (Fig. 10).  
...
identifying a weak (but significant) positive correlation ( _R_ = 0.02, *p* < 0.05; Fig. 11).
...
go_terms_worker_bp <- read.table(file = "data/output_20_fat_body_ks_log2FC_worker_classic/BP_sig.tmp",
                              header = TRUE)
go_terms_worker_bp$GO.ID <- as.character(unlist(go_terms_worker_bp$GO.ID))
nrow(go_terms_worker_bp)
go_terms_queen_bp <- read.table(file = "data/output_20_fat_body_ks_log2FC_queen_classic/BP_sig.tmp",
                              header = TRUE)
nrow(go_terms_queen_bp)
go_terms_queen_bp$GO.ID <- as.character(unlist(go_terms_queen_bp$GO.ID))
go_terms_worker_cc <- read.table(file = "data/output_20_fat_body_ks_log2FC_worker_classic/CC_sig.tmp",
                              header = TRUE)
go_terms_worker_cc$GO.ID <- as.character(unlist(go_terms_worker_cc$GO.ID))
go_terms_queen_cc <- read.table(file = "data/output_20_fat_body_ks_log2FC_queen_classic/CC_sig.tmp",
                              header = TRUE)
go_terms_queen_cc$GO.ID <- as.character(unlist(go_terms_queen_cc$GO.ID))
go_terms_worker_mf <- read.table(file = "data/output_20_fat_body_ks_log2FC_worker_classic/MF_sig.tmp",
                              header = TRUE)
go_terms_worker_mf$GO.ID <- as.character(unlist(go_terms_worker_mf$GO.ID))
go_terms_queen_mf <- read.table(file = "data/output_20_fat_body_ks_log2FC_queen_classic/MF_sig.tmp",
                              header = TRUE)
