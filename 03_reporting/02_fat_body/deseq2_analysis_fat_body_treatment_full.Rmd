...
- Insemination (n = 12; stage 1 n = 6, stage II n = 4, and stage IV n = 4)   
...
This revealed **8603** genes expressed in all samples (approximately 75% of all genes (n = 11444) expressed in the fat bodies).   
Second, density plots were generated for the percentage of expressed genes per sample for:
- Caste (Fig. 2A)
- Treatment (Fig. 2B)
- Stage (Fig. 2C)  
There was no significant difference  in terms of the mean percentage of expressed genes across castes (t-test; p = 0.07), and treatments (ANOVA; p = 0.2) but there was between individuals collected at different stages (ANOVA; p < 0.05).  
...
pdf(file = "results/fat_body_upset_plot.pdf",
    height = 12,
...
_**Fig. 2.** Density plots displaying percentage of genes expressed across samples, including: A) caste (red = "queen", blue = "worker); B) treatment (red = "control", blue = "treatment", green = "dilutant", purple = "injection"); and C) stage of ovarian development (red = "two days", green = "four days", blue = "eight days")._  

\newpage  

## Sample clustering.  
To examine general similarities and differences within and between group, a distance matrix based
on variance stabilising transformed (VST) counts was performed. This analysis highlights the presence of two samples with aberrant expression profiles (Fig. 3).    
...
To examine further similarities in terms of expression profiles across samples, a principal component analysis using VST gene values was performed. The first and second principal component, which explains 32% and 23% of the variance in the dataset, respectively, separates two individuals from the rest (Fig.4). Both individuals come from the "injection" group, with these outlier samples also appearing in the ovaries and brains. In terms of caste, PC3 seperates queens and workers (Fig. 4a), while there is no strong clustering by treatment (Fig. 4b), instead more by stage (Fig. 4c) with PC1 also separating individuals with stage IV ovarian development from those at earlier developmental stages.    
```{r, echo = FALSE, message = FALSE, results = 'hide', warning=FALSE}
...
_**Fig. 4b.** Principal component analysis with first two principal components shown. Individuals are coloured by treatment (control = "red"; inseminated group = "blue)._  
...
ggarrange(stage_plot_pc1_2,
          ...
          legend = "none")
```
_**Fig. 4c.** Principal component analysis with first two principal components shown. Individuals are coloured by treatment (two days = "blue"; four days = "green"; eight days = "red")_    
...
Based on the cluster analyses using full samples (distance matrix Fig.3; PCA Fig. 4), there are two samples that PC1 (32% of variance) separates from the others. Post-removal of the aberrant samples, PC1 explains 40% of the variance separating stage IV individuals from the rest (Fig. 5c). PC2 (26%) separates castes (Fig. 5a). There is no clear pattern associated with treatment (Fig. 5b).      
...
_**Fig. 5b.** Principal component analysis with first two principal components shown. Individuals are coloured by treatment (control = "red"; inseminated group = "blue", dilutant group = "green", injection group = "puprle")._  
...
**Model 1:** To examine the effect of **treatment** on fat body gene expression, we constructed a _full model_ consisting of:  
gene expression = ~treatment + caste + stage;   
and a _reduced model_ of:  
gene expression = ~caste + stage  

From this model, a total of **1798** genes were identified as significantly differentiated expressed (adjusted _p_ value < 0.05; Fig. 6) across treatments.  
...
_p_ value and a minimum of absolute log2 fold change of 1, our model identifies **85** (Fig. 7a)
significantly differentially expressed genes with the minority (n = **58**) having elevated 
expression in the fat bodies of inseminated bees, which is significant deviation from equal outcome of expression profiles for differentially expressed genes (binomial test; *p* < 0.05).  

**Comparison of dilutant group and control bees**: Our model identifies **84** (Fig. 7b)
significantly differentially expressed genes with half (n = **42**) having elevated 
expression in the fat bodies of bees inseminated only with dilutant.  

**Comparison of the "nothing" group and control bees**: Our model identifies **207** (Fig. 7c)
significantly differentially expressed genes with the majority (n = **171**) having elevated 
expression in the fat bodies of bees treated only with the needle.    

The overlap in terms of differentially expressed genes across treatments is shown in Fig. 8.  
...
**Model 2:** Given that we lack queen samples for the 'dilutant' and 'inhection' groups,
...
Analysis identified **3537** genes as differentially expressed (adjusted _p_ value < 0.05) between stage depending on caste. This however could be a consequence of the increased number of worker samples, including more divergent groupings.  
...
Using subsetted worker samples for only 'control' and 'insemination treatment', **2249** genes were significantly differentially expressed. Applying a threshold of absolute log2 fold change of 1, we reduce our number of differentially expressed genes to **226** (Fig. 9A).  
...
Using all queen samples, **1341** genes were differentially expressed between inseminated and control bees (Fig. 9B), which was reduced to **127** when applying a more stringent filter (Log2FC >= 1).  
...
_**Fig. 9.** Volcano plots displaying differences in gene expression of fat bodies sampled from (A) workers and (B) queens differing in insemination status. Dashed vertical lines highlight log2FC of 1 and -1, respectively. Blue dots indicate genes with significantly reduced expression (adjusted p value < 0.05 & log2FC <= -1) in fat bodies from inseminated individuals compared to control. Orange dots indicate genes with significantly elevated expression (adjusted p value < 0.05 & log2FC >= 1) in fat bodies from inseminated individuals compared to control._     

\newpage

## Euler plot:  
To visualise similarities in terms of differential expression across bumblebee castes in response to insemination, we used a euler plot. First, in response to insemination, workers and queens share **6** differentially expressed genes, which consists of approximately 3% and 7.1% of worker and queen differentially expressed genes, respectively (Fig. 10).  
...
identifying an ignorably weak, non-significant negative correlation ( _R_ = -0.01, *p*  = 0.24; Fig. 11).
...
go_terms_worker_bp <- read.table(file = "data/output_20_fat_body_ks_log2FC_worker_classic/BP_sig_new.tmp",
                              header = TRUE)
go_terms_worker_bp$GO.ID <- as.character(unlist(go_terms_worker_bp$GO.ID))
nrow(go_terms_worker_bp)
go_terms_queen_bp <- read.table(file = "data/output_20_fat_body_ks_log2FC_queen_classic/BP_sig_new.tmp",
                              header = TRUE)
nrow(go_terms_queen_bp)
go_terms_queen_bp$GO.ID <- as.character(unlist(go_terms_queen_bp$GO.ID))
go_terms_worker_cc <- read.table(file = "data/output_20_fat_body_ks_log2FC_worker_classic/CC_sig_new.tmp",
                              header = TRUE)
go_terms_worker_cc$GO.ID <- as.character(unlist(go_terms_worker_cc$GO.ID))
go_terms_queen_cc <- read.table(file = "data/output_20_fat_body_ks_log2FC_queen_classic/CC_sig_new.tmp",
                              header = TRUE)
go_terms_queen_cc$GO.ID <- as.character(unlist(go_terms_queen_cc$GO.ID))
go_terms_worker_mf <- read.table(file = "data/output_20_fat_body_ks_log2FC_worker_classic/MF_sig_new.tmp",
                              header = TRUE)
go_terms_worker_mf$GO.ID <- as.character(unlist(go_terms_worker_mf$GO.ID))
go_terms_queen_mf <- read.table(file = "data/output_20_fat_body_ks_log2FC_queen_classic/MF_sig_new.tmp",
                              header = TRUE)
...
The WGCNA identified 15 modules, of which one module ("darkorange") was significantly positively correlated with treatment (Fig. 13). However, the module was also positively correlated with caste suggesting that other factors may explain gene expression profiles better.  

```{r, echo = FALSE, message = FALSE, results = 'hide', warning=FALSE}
## Load saved R objects from the WGCNA analysis:
load(file = "data/fatbody-03-input_for_labelled_heatmap.RData")
