...
Analysis identified **1410** genes as differentially expressed (adjusted _p_ value < 0.05) between stage depending on caste. This however could be a consequence of the increased number of worker samples, including more divergent groupings.  
...
Using all queen samples, **203** genes were differentially expressed between inseminated and control bees (Fig. 9B), which was redued to **44** when applying a more stringent filter (Log2FC >= 1).  
...
go_terms_worker_bp <- read.table(file = "data/output_20_brain_ks_log2FC_worker_classic/BP_sig.tmp",
                              header = TRUE)
go_terms_worker_bp$GO.ID <- as.character(unlist(go_terms_worker_bp$GO.ID))
nrow(go_terms_worker_bp)
go_terms_queen_bp <- read.table(file = "data/output_20_brain_ks_log2FC_queen_classic/BP_sig.tmp",
                              header = TRUE)
nrow(go_terms_queen_bp)
go_terms_queen_bp$GO.ID <- as.character(unlist(go_terms_queen_bp$GO.ID))
go_terms_worker_cc <- read.table(file = "data/output_20_brain_ks_log2FC_worker_classic/CC_sig.tmp",
                              header = TRUE)
go_terms_worker_cc$GO.ID <- as.character(unlist(go_terms_worker_cc$GO.ID))
go_terms_queen_cc <- read.table(file = "data/output_20_brain_ks_log2FC_queen_classic/CC_sig.tmp",
                              header = TRUE)
go_terms_queen_cc$GO.ID <- as.character(unlist(go_terms_queen_cc$GO.ID))
go_terms_worker_mf <- read.table(file = "data/output_20_brain_ks_log2FC_worker_classic/MF_sig.tmp",
                              header = TRUE)
go_terms_worker_mf$GO.ID <- as.character(unlist(go_terms_worker_mf$GO.ID))
go_terms_queen_mf <- read.table(file = "data/output_20_brain_ks_log2FC_queen_classic/MF_sig.tmp",
                              header = TRUE)
