...
## Samples:  
The 'ovary' dataset consists of 74 samples:    
**Queen samples:** n = 26    
...
This revealed **9467** genes expressed in all samples (approximately 82% of all genes (n = 11604) expressed in the ovaries).   
Second, density plots were generated for the percentage of expressed genes per sample for:
- Caste (Fig. 2A)
- Treatment (Fig. 2B)
- Stage (Fig. 2C)  
There was no significant difference  in terms of the mean percentage of expressed genes across castes (t-test; p = 0.2), treatments (ANOVA; p = 0.3) but there was between individuals collected at different stages (ANOVA; p < 0.05).  
...
Based on the cluster analyses using full samples (distance matrix Fig.3; PCA Fig. 4), there are two samples that PC2 (3% of variance) separates from the others. Unsurprisingly, PC1 (83%) explains the vast majority of variance of the dataset separating individuals with a later ovarian development time (8 days post-activation) and differ from the rest.    
...
go_terms_worker_bp <- read.table(file = "data/output_20_ovary_ks_log2FC_worker_classic/BP_sig.tmp",
                              header = TRUE)
go_terms_worker_bp$GO.ID <- as.character(unlist(go_terms_worker_bp$GO.ID))
nrow(go_terms_worker_bp)
go_terms_queen_bp <- read.table(file = "data/output_20_ovary_ks_log2FC_queen_classic/BP_sig.tmp",
                              header = TRUE)
nrow(go_terms_queen_bp)
go_terms_queen_bp$GO.ID <- as.character(unlist(go_terms_queen_bp$GO.ID))
go_terms_worker_cc <- read.table(file = "data/output_20_ovary_ks_log2FC_worker_classic/CC_sig.tmp",
                              header = TRUE)
go_terms_worker_cc$GO.ID <- as.character(unlist(go_terms_worker_cc$GO.ID))
go_terms_queen_cc <- read.table(file = "data/output_20_ovary_ks_log2FC_queen_classic/CC_sig.tmp",
                              header = TRUE)
go_terms_queen_cc$GO.ID <- as.character(unlist(go_terms_queen_cc$GO.ID))
go_terms_worker_mf <- read.table(file = "data/output_20_ovary_ks_log2FC_worker_classic/MF_sig.tmp",
                              header = TRUE)
go_terms_worker_mf$GO.ID <- as.character(unlist(go_terms_worker_mf$GO.ID))
go_terms_queen_mf <- read.table(file = "data/output_20_ovary_ks_log2FC_queen_classic/MF_sig.tmp",
                              header = TRUE)
...
The WGCNA identified five modules, of which no module was significantly correlated with treatment (Fig. 13).   
