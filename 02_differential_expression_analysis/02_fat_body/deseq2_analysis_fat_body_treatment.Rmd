...
This revealed **9103** genes expressed in all samples (approximately 79% of all genes (n = 11557) expressed in the fat body).   
...
vsd$stage <- gsub(pattern = "_", replacement = " ", vsd$stage)
## Rename rownames:
...
pdf(file = "results/fat_body_filtered_pheatmap.pdf",
    height = 10,
    width = 10)
fat_body_heatmap_filtered <- pheatmap(sampleDistMatrix,
                                   clustering_distance_rows = sampleDists,
                                   clustering_distance_cols = sampleDists,
                                   col = colors)
dev.off()
...
_p_ value and a minimum of absolute log2 fold change of 1, our model identifies **102** (Fig. 8)
significantly differentially expressed genes with the majority (n = **96**) having elevated 
expression in the fat body of inseminated bees, which is significant deviation from equal outcome of expression profiles for differentially expressed genes (binomial test; *p* < 0.05).  
...
expression in the fat bodies of bees treated only with the needle. A complication with this 
...
pdf(file = "results/fat_body_filter_venn_diagram_degs.pdf",
    height = 10,
    width = 10)
...
Analysis identified **1419** genes as differentially expressed (adjusted _p_ value < 0.05) between stage depending on caste. This however could be a consequence of the increased number of worker samples, including more divergent groupings.  
...
identifying a weak (but significant) positive correlation ( _R_ = 0.12, *p* < 0.05; Fig. 12).
