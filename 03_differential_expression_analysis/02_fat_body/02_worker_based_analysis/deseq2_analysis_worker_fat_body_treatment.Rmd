...
## There appears to be two genes unaccounted for:
tmp <- as.data.frame(table(c(row.names(deseq_object_results_sig),
        row.names(dilutant_object_results_sig),
        row.names(injection_object_results_sig))))
  
tmp_up <- as.data.frame(table(c(row.names(deseq_object_insemination_sig_up),
                  row.names(dilutant_object_insemination_sig_up),
                  row.names(injection_object_insemination_sig_up))))

tmp_down <- as.data.frame(table(c(row.names(deseq_object_insemination_sig_down),
                  row.names(dilutant_object_insemination_sig_down),
                  row.names(injection_object_insemination_sig_down))))

tmp[, (as.character(unlist(subset(tmp,
                                  Freq == 3)$Var1)) %in% 
      c(as.character(unlist(subset(tmp_up,
                                   Freq == 3)$Var1)),
        as.character(unlist(subset(tmp_down,
                                   Freq == 3)$Var1))))]

## For genes with reduced expression in response to treatment:
venneu_treatment_comparison <- euler(combinations = list(
                                                      "AI Group I" = row.names(deseq_object_results_sig),
