...
## Samples:  
The worker 'Fat body' dataset consists:
...
This revealed **8459** genes expressed in all samples (approximately 74% of all genes (n = 11380) expressed in the fat bodies).   
...
## Save:
dir.create(path = "results/fb_worker",
           recursive = TRUE)
write.table(x = counts_df,
            file = "results/fb_worker/wfat_body_gene_level_counts.txt",
            row.names = TRUE,
            col.names = TRUE,
            quote = FALSE,
            sep = "\t")
...
pdf(file = "results/fb_worker/worker_fat_body_upset_plot.pdf",
    height = 12,
...
## Save object:
saveRDS(object = vsd,
        file = "results/fb_worker/vsd_worker_fat_body_treatment.rds")
...
pdf(file = "results/fb_worker/worker_fat_body_filtered_pheatmap.pdf",
    height = 10,
    width = 10)
...
## Save to file:
ggsave(filename = "results/fb_worker/worker_fat_body_filtered_pca_plot.pdf",
       height = 8,
       width = 8)
```
...
write.table(x = deseq_object_results,
            file = "results/fb_worker/worker_fat_body_degs.txt",
            quote = FALSE,
            row.names = TRUE,
            col.names = TRUE,
            sep = "\t")
...
table(table(c(row.names(deseq_object_results_sig),
        row.names(dilutant_object_results_sig),
        row.names(injection_object_results_sig))))
...
table(table(c(row.names(deseq_object_insemination_sig_down),
        row.names(dilutant_object_insemination_sig_down),
...
pdf(file = "results/fb_worker/worker_fat_body_all_venn_diagram_degs.pdf",
    height = 8,
    width = 8)
...
write(x = genes_of_interest,
      file = "results/fb_worker/inseminated_worker_gene_list.txt")
...
dir <- "../2022_gene_ontology_analysis/results/bterrestris_dmel/output_50_inseminated_worker_fat_body_fisher_weight01/"
## Load output:
...
## Write to file:
ggsave(filename = "results/fb_worker/combined_heatmap_barchart.pdf",
       height = 8,
       width = 12,
       dpi = 600)
```
