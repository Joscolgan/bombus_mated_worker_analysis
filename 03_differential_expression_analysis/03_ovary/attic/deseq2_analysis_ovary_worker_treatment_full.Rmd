...
## Samples:  
The 'ovary' worker dataset consists of 74 samples:  
...
This revealed **9467** genes expressed in all samples (approximately 82% of all genes (n = 11604) expressed in the fat body).   
...
## Save object:
saveRDS(object = vsd,
        file = "results/worker_only_vsd_ovary_treatment.rds")
...
## Full data:  
To examine further similarities in terms of expression profiles across samples, a principal component analysis using VST gene values was performed. The first principal component, which explains 83% of the variance in the dataset separates individual based on stage of ovarian development. PC2 explained 6% of variance which separated two individuals exposed to the injection only treatment.  
...
_p_ value and a minimum of absolute log2 fold change of 1, our model identifies **54** (Fig. 8)
significantly differentially expressed genes with the majority (n = **31**) having elevated 
expression in the ovary of inseminated bees, which was not a significant deviation from equal outcome of expression profiles for differentially expressed genes (binomial test; *p* > 0.05).  

**Comparison of dilutant group and control bees**: Our model identifies **68** (Fig. 8)
significantly differentially expressed genes with the majority (n = **42**) having reduced 
expression in the ovary of bees inseminated only with dilutant.  

**Comparison of the "nothing" group and control bees**: Our model identifies **44** (Fig. 8)
significantly differentially expressed genes with the minority (n = **8**) having elevated 
expression in the ovary of bees treated only with the needle. A complication with this 
...
saveRDS(object = deseq_object_results,
        file = "results/worker_ovary_deseq_object_results_treatment.rds")
```
...
Analysis identified **4346** genes as differentially expressed (adjusted _p_ value < 0.05) between stage depending on caste. This however could be a consequence of the increased number of worker samples, including more divergent groupings.  

```{r, echo = FALSE, message = FALSE, results = 'hide', warning=FALSE}
## Build DESeq2 dataset:
design(deseq_object) <- ~  stage +
                           treatment_group +
                           stage:treatment_group
...
## Comparison to Stage IV vs. Stage I
stageII_I_object_results <- results(deseq_object,
                                name = "stageStage_II.treatment_groupInsemination_group")

## Extract significant phenotype genes:
stageII_I_object_results_sig     <- subset(x = stageII_I_object_results,
                                             padj < 0.05 &
                                         abs(log2FoldChange) >= 1)
nrow(stageII_I_object_results_sig)

stageII_I_object_results_sig_up     <- subset(x = stageII_I_object_results,
                                             padj < 0.05 &
                                         log2FoldChange >= 1)
nrow(stageII_I_object_results_sig_up)

stageII_I_object_results_sig_down     <- subset(x = stageII_I_object_results,
                                             padj < 0.05 &
                                         log2FoldChange <= -1)
nrow(stageII_I_object_results_sig_down)
stageII_I_object_results$status <- "stageII_I"

## Comparison to Stage IV vs. Stage I
stageIV_I_object_results <- results(deseq_object,
                                name = "stageStage_IV.treatment_groupInsemination_group")

## Extract significant phenotype genes:
stageIV_I_object_results_sig     <- subset(x = stageIV_I_object_results,
                                             padj < 0.05 &
                                         abs(log2FoldChange) >= 1)
nrow(stageIV_I_object_results_sig)

stageIV_I_object_results_sig_up     <- subset(x = stageIV_I_object_results,
                                             padj < 0.05 &
                                         log2FoldChange >= 1)
nrow(stageIV_I_object_results_sig_up)

stageIV_I_object_results_sig_down     <- subset(x = stageIV_I_object_results,
                                             padj < 0.05 &
                                         log2FoldChange <= -1)
nrow(stageIV_I_object_results_sig_down)
stageIV_I_object_results$status <- "stageIV_I"

## Comparison to Stage IV vs. Stage II
## Extract results:
stageIV_II_object_results <- results(deseq_object,
                                      contrast = list("stageStage_IV.treatment_groupInsemination_group",
                                      "stageStage_II.treatment_groupInsemination_group"))

## Extract significant phenotype genes:
stageIV_II_object_results_sig     <- subset(x = stageIV_II_object_results,
                                             padj < 0.05 &
                                         abs(log2FoldChange) >= 1)
nrow(stageIV_II_object_results_sig)

stageIV_II_object_results_sig_up     <- subset(x = stageIV_II_object_results,
                                             padj < 0.05 &
                                         log2FoldChange >= 1)
nrow(stageIV_II_object_results_sig_up)

stageIV_II_object_results_sig_down     <- subset(x = stageIV_II_object_results,
                                             padj < 0.05 &
                                         log2FoldChange <= -1)
nrow(stageIV_II_object_results_sig_down)

## Convert DESeq2 result object to a dataframe:
stageIV_II_object_results <- as.data.frame(stageIV_II_object_results)
stageIV_II_object_results_sig <- as.data.frame(stageIV_II_object_results_sig)
stageIV_II_object_results$status <- "stageIV_II"

## Combine for plotting:
combined_stage_df <- as.data.frame(rbind(stageII_I_object_results,
      stageIV_I_object_results,
      stageIV_II_object_results))

insemination_plot <- ggplot(combined_stage_df,
       aes(x = log2FoldChange,
           y = -log10(padj))) +
  xlim(-10, 10) +
  geom_point(alpha = 0.5) +
  geom_point(data = subset(combined_stage_df,
                           padj < 0.05 &
                           log2FoldChange >= 1),
             aes(colour = "orange")) +
  geom_point(data = subset(combined_stage_df,
...
```{r, echo = FALSE, message = FALSE, results = 'hide', warning=FALSE}
## Compare overlap of differentially expressed genes:
venneu_intersect_up_comparison <- euler(combinations = list("Stage II vs. I" = row.names(stageII_I_object_results_sig_up),
                                                      "Stage IV vs. I" = row.names(stageIV_I_object_results_sig_up),
                                                      "Stage IV vs. II" = row.names(stageIV_II_object_results_sig_up)))

## Generate plot:
intersect_up_comparison_plot <- plot(venneu_intersect_up_comparison,
                               quantities = TRUE,
                               edges = TRUE,
                               labels = list(fontsize = 12))

venneu_intersect_dwn_comparison <- euler(combinations = list("Stage II vs. I" = row.names(stageII_I_object_results_sig_down),
                                                      "Stage IV vs. I" = row.names(stageIV_I_object_results_sig_down),
                                                      "Stage IV vs. II" = row.names(stageIV_II_object_results_sig_down)))

## Generate plot:
intersect_dwn_comparison_plot <- plot(venneu_intersect_dwn_comparison,
                               quantities = TRUE,
                               edges = TRUE,
                               labels = list(fontsize = 12))
```
...
intersect_up_comparison_plot
intersect_dwn_comparison_plot
```
...
## Comparison to Stage IV vs. Stage I
stageII_I_object_results <- results(deseq_object,
                                name = "stageStage_II.treatment_groupInsemination_with_dilutent")
...
## Comparison to Stage IV vs. Stage I
stageIV_I_object_results <- results(deseq_object,
                                name = "stageStage_IV.treatment_groupInsemination_with_dilutent")
...
## Extract results:
stageIV_II_object_results <- results(deseq_object,
                                      contrast = list("stageStage_II.treatment_groupInsemination_with_dilutent",
                                      "stageStage_IV.treatment_groupInsemination_with_dilutent"))
...
dilutant_plot <- ggplot(combined_stage_df,
       aes(x = log2FoldChange,
           y = -log10(padj))) +
  xlim(-10, 10) +
  geom_point(alpha = 0.5) +
...
## Comparison to Stage IV vs. Stage I
stageII_I_object_results <- results(deseq_object,
                                name = "stageStage_II.treatment_groupInsemination_with_nothing")
...
## Comparison to Stage IV vs. Stage I
stageIV_I_object_results <- results(deseq_object,
                                name = "stageStage_IV.treatment_groupInsemination_with_nothing")
...
## Extract results:
stageIV_II_object_results <- results(deseq_object,
                                      contrast = list("stageStage_II.treatment_groupInsemination_with_nothing",
                                      "stageStage_IV.treatment_groupInsemination_with_nothing"))
...
injection_plot <- ggplot(combined_stage_df,
       aes(x = log2FoldChange,
           y = -log10(padj))) +
  xlim(-10, 10) +
  geom_point(alpha = 0.5) +
...
stage_treatment_results <- results(stage_object,
                                 name = "stage8.treatment_groupInsemination")
