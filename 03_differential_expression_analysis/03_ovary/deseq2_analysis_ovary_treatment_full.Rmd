...
This revealed **9487** genes expressed in all samples (approximately 82% of all genes (n = 11602) expressed in the ovaries).   
Second, density plots were generated for the percentage of expressed genes per sample for:
- Caste (Fig. 2A)
- Treatment (Fig. 2B)
- Stage (Fig. 2C)  
There was no significant difference  in terms of the mean percentage of expressed genes across castes (t-test; p = 0.37), treatments (ANOVA; p = 0.4) but there was between individuals collected at different stages (ANOVA; p < 0.05).  
...
pdf(file = "results/ovary_upset_plot.pdf",
    height = 12,
...
To examine further similarities in terms of expression profiles across samples, a principal component analysis using VST gene values was performed. The first principal component, which explains 83% of the variance in the dataset and with similar patterns to the brain and fat body, clearly separates the later stage of development for the ovaries from the other time points. Similarly to the other tissues, there is evidence of two outlier samples, which are separated from the rest based on PC2.  
```{r, echo = FALSE, message = FALSE, results = 'hide', warning=FALSE}
...
Based on the cluster analyses using reduced samples with the removal of two aberrant samples (distance matrix Fig.3; PCA Fig. 4), PC1 (92%) explains the vast majority of variance of the dataset separating individuals with a later ovarian development time (8 days post-activation) and differ from the rest, which was similar to the full dataset.
...
_**Fig. 5b.** Principal component analysis with first two principal components shown. Individuals are coloured by treatment (control = "red"; inseminated group = "blue", dilutant group = "green", injection group = "purple")._  
...
**Model 1:** To examine the effect of **treatment** on ovary gene expression, we constructed a _full model_ consisting of:  
gene expression = ~treatment + caste + stage;   
and a _reduced model_ of:  
gene expression = ~caste + stage  

From this model, a total of **2406** genes were identified as significantly differentiated expressed (adjusted _p_ value < 0.05; Fig. 6) across treatments.  
...
_p_ value and a minimum of absolute log2 fold change of 1, our model identifies **103** (Fig. 7a)
significantly differentially expressed genes with the majority (n = **101**) having elevated 
expression in the ovaries of inseminated bees, which is significant deviation from equal outcome of expression profiles for differentially expressed genes (binomial test; *p* < 0.05).  

**Comparison of dilutant group and control bees**: Our model identifies **37** (Fig. 7b)
significantly differentially expressed genes with the majority (n = **32**) having elevated 
expression in the ovaries of bees inseminated only with dilutant.  

**Comparison of the "nothing" group and control bees**: Our model identifies **62** (Fig. 7c)
significantly differentially expressed genes with the majority (n = **38**) having elevated 
expression in the ovaries of bees treated only with the needle. A complication with this 
...
saveRDS(object = deseq_object_results,
        file = "results/ovary_deseq_object_results_treatment.rds")
```
...
**Model 2:** Given that we lack queen samples for the 'dilutant' and 'injection' groups,
...
Analysis identified **4521** genes as differentially expressed (adjusted _p_ value < 0.05) between stage depending on caste. This however could be a consequence of the increased number of worker samples, including more divergent groupings.  
...
Using subsetted worker samples for only 'control' and 'insemination treatment', **2913** genes were significantly differentially expressed. Applying a threshold of absolute log2 fold change of 1, we reduce our number of differentially expressed genes to **148** (Fig. 9A).  
...
saveRDS(object = worker_results_df,
        file = "results/ovary_deseq_object_results_worker_treatment.rds")
```

Using all queen samples, **2096** genes were differentially expressed between inseminated and control bees (Fig. 9B), which was redued to **49** when applying a more stringent filter (Log2FC >= 1).  
...
saveRDS(object = queen_results_df,
        file = "results/ovary_deseq_object_results_queen_treatment.rds")
```
...
_**Fig. 9.** Volcano plots displaying differences in gene expression of ovaries sampled from (A) workers and (B) queens differing in insemination status. Dashed vertical lines highlight log2FC of 1 and -1, respectively. Blue dots indicate genes with significantly reduced expression (adjusted p value < 0.05 & log2FC <= -1) in ovaries from inseminated individuals compared to control. Orange dots indicate genes with significantly elevated expression (adjusted p value < 0.05 & log2FC >= 1) in ovaries from inseminated individuals compared to control._     

\newpage

## Euler plot:  
To visualise similarities in terms of differential expression across bumblebee castes in response to insemination, we used a euler plot. First, in response to insemination, workers and queens share **5** differentially expressed genes, which consists of approximately 3% and 10% of worker and queen differentially expressed genes, respectively (Fig. 10).  
...
identifying an weak correlation ( _R_ = 0.05, *p* < 0.05; Fig. 11).
...
For both worker and queen datasets, significantly enriched GO terms (n = 444 and 154, respectively)  
were examined for overlap using a Euler plot identifying 72 enriched GO terms shared amongst
...
go_terms_worker_bp <- read.table(file = "data/output_20_ovary_ks_log2FC_worker_classic/BP_sig_new.tmp",
                              header = TRUE)
go_terms_worker_bp$GO.ID <- as.character(unlist(go_terms_worker_bp$GO.ID))
nrow(go_terms_worker_bp)
go_terms_queen_bp <- read.table(file = "data/output_20_ovary_ks_log2FC_queen_classic/BP_sig_new.tmp",
                              header = TRUE)
nrow(go_terms_queen_bp)
go_terms_queen_bp$GO.ID <- as.character(unlist(go_terms_queen_bp$GO.ID))
go_terms_worker_cc <- read.table(file = "data/output_20_ovary_ks_log2FC_worker_classic/CC_sig_new.tmp",
                              header = TRUE)
go_terms_worker_cc$GO.ID <- as.character(unlist(go_terms_worker_cc$GO.ID))
go_terms_queen_cc <- read.table(file = "data/output_20_ovary_ks_log2FC_queen_classic/CC_sig_new.tmp",
                              header = TRUE)
go_terms_queen_cc$GO.ID <- as.character(unlist(go_terms_queen_cc$GO.ID))
go_terms_worker_mf <- read.table(file = "data/output_20_ovary_ks_log2FC_worker_classic/MF_sig_new.tmp",
                              header = TRUE)
go_terms_worker_mf$GO.ID <- as.character(unlist(go_terms_worker_mf$GO.ID))
go_terms_queen_mf <- read.table(file = "data/output_20_ovary_ks_log2FC_queen_classic/MF_sig_new.tmp",
                              header = TRUE)
...
The WGCNA identified five modules, of which no module was significantly positively correlated with treatment (Fig. 13).   

```{r, echo = FALSE, message = FALSE, results = 'hide', warning=FALSE}
## Load saved R objects from the WGCNA analysis:
load(file = "data/ovary-03-input_for_labelled_heatmap.RData")
#load(file = "data/spermatheca-04-input_for_membership_scatterplot.RData")
```

```{r figure18, ref.label = "figure18", fig.width = 6, fig.height = 4, fig.align = 'center',, echo = FALSE, message=FALSE, warning=FALSE}
par(mar = c(6, 8.5, 3, 3));
