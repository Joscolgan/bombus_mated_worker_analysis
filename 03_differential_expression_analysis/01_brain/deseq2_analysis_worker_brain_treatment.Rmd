...
pdf(file = "results/worker_upset_plot.pdf",
    height = 12,
...
## Save object:
saveRDS(object = vsd,
        file = "results/vsd_worker_brain_treatment.rds")
...
## Save to file:
ggsave(filename = "results/worker_brain_filtered_pca_plot.pdf",
       height = 8,
       width = 8)
```
...
saveRDS(object = deseq_object,
        file = "results/workers_deseq2_object.rds")
...
write.table(x = deseq_object_results,
            file = "results/worker_brain_insem_degs.txt",
            quote = FALSE,
            row.names = TRUE,
            col.names = TRUE,
            sep = "\t")
...
table(table(c(row.names(deseq_object_insemination_sig_up),
              row.names(dilutant_object_insemination_sig_up),
              row.names(injection_object_insemination_sig_up))))

insemination_unique <- subset(x = deseq_object_insemination_sig_up,
       !(row.names(deseq_object_insemination_sig_up) %in% 
         c(row.names(dilutant_object_insemination_sig_up),
           row.names(injection_object_insemination_sig_up))))
...
pdf(file = "results/brain_worker/worker_brain_filter_elevated_degs_venn_diagram_degs.pdf",
    height = 10,
    width = 10)
...
saveRDS(object = deseq_object_results,
        file = "results/brain_worker/worker_deseq_object_results_treatment.rds")
...
pdf(file = "results/brain_worker/worker_brain_filter_reduced_degs_venn_diagram_degs.pdf",
    height = 10,
    width = 10)
treatment_comparison_plot
dev.off()

insemination_down_unique <- subset(x = deseq_object_insemination_sig_down,
                                   !(row.names(deseq_object_insemination_sig_down) %in% 
                                       c(row.names(dilutant_object_insemination_sig_down),
                                         row.names(injection_object_insemination_sig_down))))
...
counts_subset_df <- subset(x = counts_df,
       row.names(counts_df) %in% c(row.names(insemination_unique),
                                   row.names(insemination_down_unique)))
...
write(x = genes_of_interest,
      file = "results/brain_worker/inseminated_worker_elevated_gene_list.txt")

select <- row.names(counts_df) %in% genes_of_interest

select <- order(rowMeans(counts(dds_htseq,normalized=FALSE)),
                decreasing=TRUE)[1:20]
df <- as.data.frame(colData(dds_htseq)[,c("stage","treatment_group")])
pheatmap(assay(vsd)[select, 23:45],
         cluster_rows=TRUE,
         show_rownames=FALSE,
         cluster_cols=FALSE,
         annotation_col=df)
