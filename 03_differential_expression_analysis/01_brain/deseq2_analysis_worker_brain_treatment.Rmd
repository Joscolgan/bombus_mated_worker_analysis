...
```{r, echo = FALSE, message = FALSE, results = 'hide', warning=FALSE}
# Step Three: Put sample names and treatments into a samples dataframe for
...
4. Examine gene-level outlier samples:

```{r, echo = FALSE, message = FALSE, results = 'hide', warning=FALSE}
#Step Three: Explore gene-level accounts across samples to identify potential
# outliers.

#The level of alignment across samples is generally quite good (>75% of all
#genes detected across samples (Fig. 1) with on average (mean) 87.5% of genes
# detected across all samples) (Figs.2-4).
...
write.table(x = counts_df,
            file = "results/brain_worker/wbrain_gene_level_counts.txt",
            row.names = TRUE,
            col.names = TRUE,
            quote = FALSE,
            sep = "\t")

## For next steps, remove lowly expressed genes:
...
pdf(file = "results/brain_worker/worker_upset_plot.pdf",
    height = 12,
...
5. Generate UpSet plot of gene-level counts across samples:
...
7. Generate a combined density plot:
...
## Save object:
saveRDS(object = vsd,
        file = "results/brain_worker/vsd_worker_brain_treatment.rds")

sample_dists <- dist(t(assay(vsd)))

sample_dists_matrix <- as.matrix(sample_dists)
rownames(sample_dists_matrix) <- paste(vsd$caste,
                                    vsd$treatment_group,
                                    vsd$stage,
                                    sep = " - ")
colnames(sample_dists_matrix) <- NULL
colors <- colorRampPalette(rev(brewer.pal(9, "Blues")))(255)
```

9. Create input files for distance matrix:

```{r figure3, ref.label = "figure3", fig.width = 8, fig.height = 6, echo = FALSE, message=FALSE}
## Rename rownames:
rownames(sample_dists_matrix) <- gsub(pattern = "Insemination_group",
                                   replacement = "AI Group 1",
                                   row.names(sample_dists_matrix))

rownames(sample_dists_matrix) <- gsub(pattern = "Insemination_with_dilutent",
                                   replacement = "AI Group 2",
                                   row.names(sample_dists_matrix))

rownames(sample_dists_matrix) <- gsub(pattern = "Insemination_with_nothing",
                                   replacement = "AI Group 3",
                                   row.names(sample_dists_matrix))

pheatmap(sample_dists_matrix,
...
10. Visualisation of PCA output:  
...
sample_dists_matrix <- as.matrix(sample_dists)
rownames(sample_dists_matrix) <- paste(vsd$treatment_group,
                                       vsd$stage,
                                       sep = " - ")
colnames(sample_dists_matrix) <- NULL
colors <- colorRampPalette(rev(brewer.pal(9, "Blues")))(255)
...
pdf(file = "results/brain_worker/brain_filtered_pheatmap.pdf",
    height = 10,
    width = 10)
brain_heatmap_filtered <- pheatmap(sample_dists_matrix,
                                   clustering_distance_rows = sample_dists,
                                   clustering_distance_cols = sample_dists,
                                   col = colors)
dev.off()
...
## Save to file:
ggsave(filename = "results/brain_worker/worker_brain_filtered_pca_plot.pdf",
       height = 8,
       width = 8)
```
...
saveRDS(object = deseq_object,
        file = "results/brain_worker/workers_deseq2_object.rds")
...
write.table(x = deseq_object_results,
            file = "results/brain_worker/worker_brain_degs.txt",
            quote = FALSE,
            row.names = TRUE,
            col.names = TRUE,
            sep = "\t")
...
## Subset genes with evidence of significantly elevated expression in
## inseminated individuals:
deseq_object_insemination_sig_up     <- subset(x = deseq_object_results,
                                             padj < 0.05 &
                                            log2FoldChange >= 1)
nrow(deseq_object_insemination_sig_up)
## Write genes to file:
dir.create(path = "results/brain_worker",
           recursive = TRUE)
write.table(x = deseq_object_insemination_sig_up,
            file = "results/brain_worker/deseq_object_insemination_sig_up.txt",
            quote = FALSE,
            sep = "\t",
            col.names = TRUE,
            row.names = TRUE)

## Subset genes with evidence of significantly reduced expression in inseminated
## individuals:
deseq_object_insemination_sig_down <- subset(x = deseq_object_results,
                                             padj < 0.05 &
                                             log2FoldChange <= -1)
nrow(deseq_object_insemination_sig_down)
write.table(x = deseq_object_insemination_sig_down,
          file = "results/brain_worker/deseq_object_insemination_sig_down.txt",
          quote = FALSE,
          sep = "\t",
          col.names = TRUE,
          row.names = TRUE)
...
venneu_treatment_comparison <- euler(combinations = list(
                                                      "AI Group 1" = row.names(deseq_object_results_sig),
                                                      "AI Group 2" = row.names(dilutant_object_results_sig),
                                                      "AI Group 3" = row.names(injection_object_results_sig)),
                                     shape = "ellipse")
## Generate plot:
...
pdf(file = "results/brain_worker/worker_brain_all_venn_diagram_degs.pdf",
    height = 8,
    width = 8)
treatment_comparison_plot
dev.off()
insemination_unique <- subset(x = deseq_object_results_sig,
                                   !(row.names(deseq_object_results_sig) %in% 
                                       c(row.names(dilutant_object_results_sig),
                                         row.names(injection_object_results_sig))))
## Subset rows of interest:
counts_df <- as.data.frame(counts(dds_htseq))
counts_subset_df <- subset(x = counts_df,
       row.names(counts_df) %in% insemination_unique)
head(assay(vsd))
...
write(x = genes_of_interest,
      file = "results/brain_worker/inseminated_worker_unique_gene_list.txt")
...
## Subset worker info to reorder:
samples_info_worker <- subset(x = samples_info,
                              caste == "Worker")
samples_info_worker_sorted <- samples_info_worker[with(samples_info_worker,
                                                       order(samples_info_worker$stage,
                                                             samples_info_worker$treatment_group)), ]
input$sample <- factor(input$sample,
                          levels = unique(samples_info_worker_sorted$short_name))
dim(input)
## Update sample names for plotting:
for (name in 1:nrow(input)){
  print(input$sample[[name]])
  tmp <- as.character(unlist(input$sample[[name]]))
  input$test[[name]]    <-            c(paste(subset(x = samples_info_worker,
                          short_name == tmp)$treatment_group,
                   " ",
                   subset(x = samples_info_worker,
                          short_name == tmp)$stage,
                   " (",
                   subset(x = samples_info_worker,
...
input$test <- gsub(pattern = "Stage_II",
                   replacement = "Stage II",
                   input$test)
input$test <- gsub(pattern = "Stage_IV",
                   replacement = "Stage IV",
                   input$test)
input$test <- gsub(pattern = "Stage_I",
                   replacement = "Stage I",
                   input$test)
input$test <- gsub(pattern = "Insemination_group",
                   replacement = "Inseminated",
                   input$test)
...
dir <- "../2022_gene_ontology_analysis/results/bterrestris_dmel/output_50_inseminated_worker_brain_fisher_weight01/"
## Load output:
...
## Write to file:
ggsave(filename = "results/brain_worker/combined_heatmap_barchart.pdf",
       height = 8,
       width = 12,
       dpi = 600)
```
...
```{r, message = FALSE}
lintr::lint(file = "deseq2_analysis_worker_brain_treatment.Rmd")
```
