...
## Generate plot:
treatment_up_comparison_plot <- plot(venneu_treatment_comparison,
                               quantities = TRUE,
                               edges = TRUE,
                               labels = list(fontsize = 12))
## Print:
treatment_up_comparison_plot
pdf(file = "results/worker_brain_filter_elevated_degs_venn_diagram_degs.pdf",
    height = 10,
    width = 10)
treatment_up_comparison_plot
dev.off()

## For genes with reduced expression in response to treatment:
venneu_treatment_comparison <- euler(combinations = list(
                                                      "AI Group I DEGs" = row.names(deseq_object_results_sig),
                                                      "AI Group 2 DEGs" = row.names(dilutant_object_results_sig),
                                                      "AI Group 3 DEGs" = row.names(injection_object_results_sig)),
                                     shape = "ellipse")
## Generate plot:
...
genes_of_interest <- c(row.names(insemination_unique),
                                   row.names(insemination_down_unique))
## Write to file:
dir.create(path = "results/brain/")
write(x = genes_of_interest,
      file = "results/brain/inseminated_worker_elevated_gene_list.txt")
